...
from cricfree_links import get_scraper as get_cricfree_scraper
...
def get_all_events():
    ...
    # Sportsurge
    try:
        ss = SportsurgeScraper()
        html, _ = ss.fetch("https://sportsurge.ws/")
        events.extend(ss.get_homepage_events(html))
    except Exception as e:
        app.logger.error(f"Sportsurge scrape failed: {e}")
    # Cricfree
    try:
        cs = get_cricfree_scraper()
        events.extend(cs.get_homepage_events())
    except Exception as e:
        app.logger.error(f"Cricfree scrape failed: {e}")
    return events


def get_embed_urls(watch_url):
    # Try Sportsurge first
    try:
        ss = SportsurgeScraper()
        entries = ss.get_embed_urls(watch_url)
        if entries:
            return entries
    except Exception as e:
        app.logger.debug(f"Sportsurge embed failed: {e}")
    # Try Cricfree
    try:
        cs = get_cricfree_scraper()
        entries = cs.get_embed_urls(watch_url)
        if entries:
            return entries
    except Exception as e:
        app.logger.debug(f"Cricfree embed failed: {e}")
    return []
...
def event_page():
    ...
    entries = get_embed_urls(watch_url)
    if not entries:
        return "No streams found", 404
    template = """
    <!doctype html>
